...
- `docker version`, `docker compose version`, and `docker info` passed before startup.
- Ports `8080`, `8888`, `3306`, and `6379` were confirmed free before startup.
- `.runtime/`, `.runtime/admin-bootstrap.env`, `.runtime/admin-compose.env`, and `.runtime/admin-login.env` were confirmed ignored by Git; runtime files were mode `0600` and directory mode `0700`.
- `docker compose --env-file .runtime/admin-bootstrap.env -f deploy/production/docker-compose.yml config >/dev/null` passed.
- `docker compose --env-file .runtime/admin-bootstrap.env -f deploy/production/docker-compose.yml down --volumes --remove-orphans` completed before the clean runtime start.
- `docker compose --env-file .runtime/admin-bootstrap.env -f deploy/production/docker-compose.yml build --build-arg GOPROXY=https://goproxy.cn,direct app-api` passed.
- `docker compose --env-file .runtime/admin-bootstrap.env -f deploy/production/docker-compose.yml build --build-arg NPM_CONFIG_REGISTRY=https://registry.npmmirror.com admin-web` passed, including Vue type checking and production build inside the Docker build.
- `docker compose --env-file .runtime/admin-bootstrap.env -f deploy/production/docker-compose.yml up -d --no-build --wait` passed.
- `curl --fail --silent --show-error http://127.0.0.1:8888/health/live` returned `{"status":"ok"}`.
- `curl --fail --silent --show-error http://127.0.0.1:8888/health/ready` returned `{"status":"ready"}`.
- `curl --fail --silent --show-error http://127.0.0.1:8080/health` returned `ok`.
- `curl --fail --silent --show-error http://127.0.0.1:8080/ >/dev/null` passed.
- Bootstrap status was initially available, first administrator creation succeeded, bootstrap replay returned HTTP 409, and bootstrap status became unavailable.
- Authenticated smoke checks passed for `/auth/session`, `/admin/me`, `/admin/roles`, `/admin/authorization/resources`, `/admin/authorization/engine`, and `/admin/system/configuration`.
- Admin Web reverse proxy check `http://127.0.0.1:8080/api/health/live` returned the API live status.
