...
func TestIntegrationDependenciesAndHealth(t *testing.T) {
	...
	if schemaVersion != "0009" {
		t.Fatalf("schema version = %q, want 0009", schemaVersion)
	}
	...
	if err := app.mysql.DB().QueryRowContext(ctx, "SELECT version FROM authorization_policy_state WHERE policy_key = ?", "global").Scan(&policyVersion); err != nil {
		t.Fatalf("query authorization policy version: %v", err)
	}
	if policyVersion < 1 {
		t.Fatalf("authorization policy version = %d, want at least 1", policyVersion)
	}
