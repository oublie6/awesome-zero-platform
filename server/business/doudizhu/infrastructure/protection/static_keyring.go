...
func NewStaticKeyring(current string, keys map[string][]byte) (*StaticKeyring, error) {
	if current == "" || len(keys) == 0 {
		return nil, fmt.Errorf("%w: static keyring configuration", ErrKeyUnavailable)
	}
	copied := make(map[string][]byte, len(keys))
	for id, key := range keys {
		if id == "" || len(key) != 32 {
			return nil, fmt.Errorf("%w: key %q", ErrKeyUnavailable, id)
		}
		copied[id] = append([]byte(nil), key...)
	}
	if _, ok := copied[current]; !ok {
		return nil, fmt.Errorf("%w: current key", ErrKeyUnavailable)
	}
